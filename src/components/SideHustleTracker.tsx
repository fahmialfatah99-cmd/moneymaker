      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-slate-400">Rate/Jam</span>
          </div>
        ...
        {showForm && (
          <div className="mb-6 p-4 rounded-xl bg-slate-800/50 border border-indigo-500/20 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <input type="text" placeholder="Nama side hustle" value={newEntry.name}
              ...
              <input type="number" placeholder="Income (Rp)" value={newEntry.income || ''}
                onChange={e => setNewEntry({...newEntry, income: parseInt(e.target.value) || 0})}
                className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white text-sm focus:border-indigo-500 focus:outline-none placeholder:text-slate-600" />
              <input type="number" placeholder="Expense (Rp)" value={newEntry.expense || ''}
                onChange={e => setNewEntry({...newEntry, expense: parseInt(e.target.value) || 0})}
                className="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white text-sm focus:border-indigo-500 focus:outline-none placeholder:text-slate-600" />
              <input type="number" placeholder="Jam/bulan" value={newEntry.hours || ''}
            ...
            <div className="flex gap-2">
              <button onClick={addEntry} className="px-4 py-2 rounded-lg bg-emerald-500/20 text-emerald-300 text-sm hover:bg-emerald-500/30 transition-all">
                Simpan
              </button>
              <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-lg bg-slate-700/50 text-slate-300 text-sm hover:bg-slate-700 transition-all">
                Batal
              </button>
            </div>
          </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    entry.status === 'active' ? 'bg-emerald-500/10 text-emerald-400' :
                    entry.status === 'paused' ? 'bg-amber-500/10 text-amber-400' :
